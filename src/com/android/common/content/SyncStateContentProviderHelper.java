...
/**
 * Extends the schema of a ContentProvider to include the _sync_state table and
 * implements query/insert/update/delete to access that table using the
 * authority "syncstate". This can be used to store the sync state for a set of
 * accounts.
 */
@TargetApi(Build.VERSION_CODES.ECLAIR)
public class SyncStateContentProviderHelper {
	private static final String SELECT_BY_ACCOUNT = SyncStateContract.Columns.ACCOUNT_NAME + "=? AND "
	...
	private static final String[] ACCOUNT_PROJECTION = new String[] { SyncStateContract.Columns.ACCOUNT_NAME,
			SyncStateContract.Columns.ACCOUNT_TYPE };

	public static final String PATH = "syncstate";

	private static final String QUERY_COUNT_SYNC_STATE_ROWS = "SELECT count(*)" + " FROM " + SYNC_STATE_TABLE + " WHERE "
			+ SyncStateContract.Columns._ID + "=?";

	public void createDatabase(SQLiteDatabase db) {
		db.execSQL("DROP TABLE IF EXISTS " + SYNC_STATE_TABLE);
		db.execSQL("CREATE TABLE " + SYNC_STATE_TABLE + " (" + SyncStateContract.Columns._ID + " INTEGER PRIMARY KEY,"
				+ SyncStateContract.Columns.ACCOUNT_NAME + " TEXT NOT NULL," + SyncStateContract.Columns.ACCOUNT_TYPE + " TEXT NOT NULL,"
				+ SyncStateContract.Columns.DATA + " TEXT," + "UNIQUE(" + SyncStateContract.Columns.ACCOUNT_NAME + ", "
				+ SyncStateContract.Columns.ACCOUNT_TYPE + "));");
	...
	public void onDatabaseOpened(SQLiteDatabase db) {
		long version = DatabaseUtils.longForQuery(db, "SELECT " + SYNC_STATE_META_VERSION_COLUMN + " FROM " + SYNC_STATE_META_TABLE, null);
	...
	public int update(SQLiteDatabase db, long rowId, Object data) {
		...
		db.execSQL("UPDATE " + SYNC_STATE_TABLE + " SET " + SyncStateContract.Columns.DATA + "=?" + " WHERE "
				+ SyncStateContract.Columns._ID + "=" + rowId, new Object[] { data });
