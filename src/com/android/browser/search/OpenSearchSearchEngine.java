...
public class OpenSearchSearchEngine implements SearchEngine {
	...
	// The suggestion columns used. If you are adding a new entry to these
	// arrays make sure to
	// update the list of indices declared above.
	private static final String[] COLUMNS = new String[] { "_id", SearchManager.SUGGEST_COLUMN_QUERY, SearchManager.SUGGEST_COLUMN_ICON_1,
			SearchManager.SUGGEST_COLUMN_TEXT_1, SearchManager.SUGGEST_COLUMN_TEXT_2, };

	private static final String[] COLUMNS_WITHOUT_DESCRIPTION = new String[] { "_id", SearchManager.SUGGEST_COLUMN_QUERY,
	...
	public Cursor getSuggestions(Context context, String query) {
		...
		try {
			...
			if (content == null)
				return null;
			/*
			 * The data format is a JSON array with items being regular strings
			 * or JSON arrays themselves. We are interested in the second and
			 * third elements, both of which should be JSON arrays. The second
			 * element/array contains the suggestions and the third element
			 * contains the descriptions. Some search engines don't support
			...
			if (results.length() > 2) {
				...
				// Some search engines given an empty array "[]" for
