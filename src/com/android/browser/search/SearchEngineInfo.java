...
public class SearchEngineInfo {
	...
	// The fields of a search engine data array, defined in the same order as
	// they appear in the
	// all_search_engines.xml file.
	// If you are adding/removing to this list, remember to update NUM_FIELDS
	// below.
	...
	// The array of strings defining this search engine. The array values are in
	// the same order as
	 ...
	 *             If the name does not refer to a valid search engine
	 */
	public SearchEngineInfo(Context context, String name) throws IllegalArgumentException {
		...
		mSearchEngineData[FIELD_SEARCH_URI] = mSearchEngineData[FIELD_SEARCH_URI].replace(PARAMETER_INPUT_ENCODING, enc);
		mSearchEngineData[FIELD_SUGGEST_URI] = mSearchEngineData[FIELD_SUGGEST_URI].replace(PARAMETER_INPUT_ENCODING, enc);
	}
	...
	/**
	 * Returns the URI for launching a web search with the given query (or null
	 * if there was no data available for this search engine).
	 */
	...
	/**
	 * Returns the URI for retrieving web search suggestions for the given query
	 * (or null if there was no data available for this search engine).
	 */
	...
	/**
	 * Formats a launchable uri out of the template uri by replacing the
	...
	private String getFormattedUri(String templateUri, String query) {
		...
		// Encode the query terms in the requested encoding (and fallback to
		// UTF-8 if not).
		String enc = mSearchEngineData[FIELD_ENCODING];
		try {
