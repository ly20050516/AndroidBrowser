...
public class GeolocationPermissionsPrompt extends RelativeLayout {
	...
	/**
	 * Shows the prompt for the given origin. When the user clicks on one of the
	 * buttons, the supplied callback is be called.
	 */
	...
	private void handleButtonClick(boolean allow) {
		...
		if (remember) {
			Toast toast = Toast.makeText(getContext(), allow ? R.string.geolocation_permissions_prompt_toast_allowed
					: R.string.geolocation_permissions_prompt_toast_disallowed, Toast.LENGTH_LONG);
	...
	private void setMessage(CharSequence origin) {
		mMessage.setText(String.format(getResources().getString(R.string.geolocation_permissions_prompt_message), origin));
	}
}
