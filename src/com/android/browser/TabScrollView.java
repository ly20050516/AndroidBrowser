...
public class TabScrollView extends HorizontalScrollView {
	...
	private void init(Context ctx) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " TabScrollView init(Context ctx)");
		mAnimationDuration = ctx.getResources().getInteger(R.integer.tab_animation_duration);
		...
		mContentView.setPadding((int) ctx.getResources().getDimension(R.dimen.tab_first_padding_left), 0, 0, 0);
	...
	// TODO: These animations are broken and don't work correctly, removing for
	// now as animateOut is actually causing issues
	@SuppressLint("NewApi")
	...
	private void animateOut(final View tab) {
		ObjectAnimator animator = ObjectAnimator.ofInt(tab, "TranslationX", 0, getScrollX() - tab.getRight());
		animator.setDuration(mAnimationDuration);
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		// TabViews base their drawing based on their absolute position within
		// the
		// window. When hardware accelerated, we need to recreate their display
		// list
		// when they scroll
	...
	class TabLayout extends LinearLayout {
		...
		public TabLayout(Context context) {
			...
			Log.d("Liu Test", getClass().getSimpleName() + " TabLayout(Context context)");
