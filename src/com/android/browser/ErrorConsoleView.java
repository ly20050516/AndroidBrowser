...
/* package */class ErrorConsoleView extends LinearLayout {
	...
	// Before we've been asked to display the console, cache any messages that
	// should
	// be added to the console. Then when we do display the console, add them to
	// the view
	// then.
	...
	private void commonSetupIfNeeded() {
		...
		mEvalButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				// Send the javascript to be evaluated to webkit as a
				// javascript: url
				// TODO: Can we expose access to webkit's JS interpreter here
				// and evaluate it that
				// way? Note that this is called on the UI thread so we will
		...
		// Add any cached messages to the list now that we've assembled the
		// view.
	...
	/**
	 * Sets the webview that this console is associated with. Currently this is
	 * used so we can call into webkit to evaluate JS expressions in the
	 * console.
	 */
	 ...
	 * display the set of errors encountered on the current page.
	 */
	private static class ErrorConsoleListView extends ListView {
		...
		/**
		 * This class is an adapter for ErrorConsoleListView that contains the
		 * error console message data.
		 */
		private static class ErrorConsoleMessageList extends android.widget.BaseAdapter implements android.widget.ListAdapter {
