...
public class BookmarkUtils {
	...
	enum BookmarkIconType {
		ICON_INSTALLABLE_WEB_APP, // Icon for an installable web app (launches
									// WebAppRuntime).
		ICON_HOME_SHORTCUT, // Icon for a shortcut on the home screen (launches
							// Browser).
		ICON_WIDGET,
	}

	/**
	 * Creates an icon to be associated with this bookmark. If available, the
	 * apple touch icon will be used, else we draw our own depending on the type
	 * of "bookmark" being created.
	 */
	...
	private static Bitmap createIcon(Context context, Bitmap touchIcon, Bitmap favicon, BookmarkIconType type, int iconDimension,
			int iconDensity) {
		...
		if (touchIcon != null) {
			...
			// Set the background based on the type of shortcut (either webapp
			// or home shortcut).
			...
			// If we have a favicon, overlay it in a nice rounded white box on
			// top of the
			// background.
			if (favicon != null) {
	...
	/**
	 * Convenience method for creating an intent that will add a shortcut to the
	 * home screen.
	 */
	static Intent createAddToHomeIntent(Context context, String url, String title, Bitmap touchIcon, Bitmap favicon) {
		...
		i.putExtra(Intent.EXTRA_SHORTCUT_ICON, createIcon(context, touchIcon, favicon, BookmarkIconType.ICON_HOME_SHORTCUT));
	...
	private static Bitmap getIconBackground(Context context, BookmarkIconType type, int density) {
		if (type == BookmarkIconType.ICON_HOME_SHORTCUT) {
			...
			// Use the web browser icon as the background for the icon for an
			// installable
			// web app.
			Drawable drawable = context.getResources().getDrawableForDensity(R.mipmap.ic_launcher_browser, density);
			if (drawable instanceof BitmapDrawable) {
				BitmapDrawable bd = (BitmapDrawable) drawable;
				return bd.getBitmap();
			}
	 ...
	 * @param title
	 *            Title of the bookmark, to be displayed in the confirmation
	 *            method.
	 * @param context
	 *            Package Context for strings, dialog, ContentResolver
	 * @param msg
	 *            Message to send if the bookmark is deleted.
	 */
	static void displayRemoveBookmarkDialog(final long id, final String title, final Context context, final Message msg) {
