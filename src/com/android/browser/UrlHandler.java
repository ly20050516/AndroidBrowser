...
public class UrlHandler {
	...
	boolean handleMenuClick(Tab tab, String url) {
		if (mController.isMenuDown()) {
			mController.openTab(url, (tab != null) && tab.isPrivateBrowsingEnabled(), !BrowserSettings.getInstance().openInBackground(),
					true);
	...
	private class RLZTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... unused) {
			...
			Cursor cur = null;
			try {
				cur = mActivity.getContentResolver().query(getRlzUri(), null, null, null, null);
				if (cur != null && cur.moveToFirst() && !cur.isNull(0)) {
					result = mSiteUri.buildUpon().appendQueryParameter("rlz", cur.getString(0)).build().toString();
				}
			} finally {
				if (cur != null) {
					cur.close();
		...
		protected void onPostExecute(String result) {
			...
			if (mController.isActivityPaused())
				return;
	...
	private Uri getRlzUri() {
		if (mRlzUri == null) {
			String ap = mActivity.getResources().getString(R.string.rlz_access_point);
	...
	private static boolean needsRlzString(Uri uri) {
		...
		if (("http".equals(scheme) || "https".equals(scheme)) && (uri.getQueryParameter("q") != null)
