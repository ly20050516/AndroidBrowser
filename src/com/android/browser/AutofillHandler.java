...
public class AutofillHandler {
	...
	/**
	 * Load settings from the browser app's database. It is performed in an
	 * AsyncTask as it involves plenty of slow disk IO. NOTE: Strings used for
	 * the preferences must match those specified in the various preference XML
	 * files.
	 */
	...
	private class LoadFromDb extends Thread {
		...
		@Override
		public void run() {
			// Note the lack of synchronization over mAutoFillActiveProfileId
			// and
			// mAutoFillProfile here. This is because we control all other
			// access
			// to these members through the public functions of this class, and
			// they
			...
			mAutoFillActiveProfileId = p.getInt(PreferenceKeys.PREF_AUTOFILL_ACTIVE_PROFILE_ID, mAutoFillActiveProfileId);

			// Load the autofill profile data from the database. We use a
			// database separate
			// to the browser preference DB to make it easier to support
			// multiple profiles
			// and switching between them. Note that this may block startup if
			// this DB lookup
			// is extremely slow. We do this to ensure that if there's a profile
			// set, the
			...
			if (c.getCount() > 0) {
				...
				String fullName = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.FULL_NAME));
				String email = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.EMAIL_ADDRESS));
				String company = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.COMPANY_NAME));
				String addressLine1 = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.ADDRESS_LINE_1));
				String addressLine2 = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.ADDRESS_LINE_2));
				String city = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.CITY));
				String state = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.STATE));
				String zip = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.ZIP_CODE));
				String country = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.COUNTRY));
				String phone = c.getString(c.getColumnIndex(AutoFillProfileDatabase.Profiles.PHONE_NUMBER));
				mAutoFillProfile = new AutoFillProfile(mAutoFillActiveProfileId, fullName, email, company, addressLine1, addressLine2,
						city, state, zip, country, phone);
			}
			c.close();
			autoFillDb.close();

			// At this point we've loaded the profile if there was one, so let
			// any thread
			// waiting on initialization continue.
			mLoaded.countDown();

			// Synchronization note: strictly speaking, it's possible that
			// mAutoFillProfile
			// may get a value after we check below, but that's OK. This check
			// is only an
			// optimisation, and we do a proper synchronized check further down
			// when it comes
			// to actually setting the inferred profile.
			if (mAutoFillProfile == null) {
				// We did not load a profile from disk. Try to infer one from
				// the user's
				// "me" contact.
				final Uri profileUri = Uri.withAppendedPath(ContactsContract.Profile.CONTENT_URI,
						ContactsContract.Contacts.Data.CONTENT_DIRECTORY);
				String name = getContactField(profileUri, ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME,
						ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE);
				// Only attempt to read other data and set a profile if we could
				// successfully
				// get a name.
				if (name != null) {
					String email = getContactField(profileUri, ContactsContract.CommonDataKinds.Email.ADDRESS,
							ContactsContract.CommonDataKinds.Email.CONTENT_ITEM_TYPE);
					String phone = getContactField(profileUri, ContactsContract.CommonDataKinds.Phone.NUMBER,
							ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE);
					String company = getContactField(profileUri, ContactsContract.CommonDataKinds.Organization.COMPANY,
							ContactsContract.CommonDataKinds.Organization.CONTENT_ITEM_TYPE);

					// Can't easily get structured postal address information
					// (even using
					// CommonDataKinds.StructuredPostal) so omit prepopulating
					// that for now.
					// When querying structured postal data, it often all comes
					// back as a string
					// inside the "street" field.

					synchronized (AutofillHandler.this) {
						// Only use this profile if one hasn't been set
						// inbetween the
						// inital import and this thread getting to this point.
						if (mAutoFillProfile == null) {
							setAutoFillProfile(new AutoFillProfile(1, name, email, company, null, null, null, null, null, null, phone),
									null);
						}
					}
				}
		...
		private String getContactField(Uri uri, String field, String itemType) {
			...
			Cursor c = mContext.getContentResolver().query(uri, new String[] { field }, ContactsContract.Data.MIMETYPE + "=?",
					new String[] { itemType }, null);
	...
	public synchronized void setAutoFillProfile(AutoFillProfile profile, Message msg) {
		...
		// Make sure we set mAutoFillProfile before calling
	...
	private synchronized void setActiveAutoFillProfileId(int activeProfileId) {
		...
		Editor ed = PreferenceManager.getDefaultSharedPreferences(mContext).edit();
