...
public class AddBookmarkPage extends Activity implements View.OnClickListener, TextView.OnEditorActionListener,
		AdapterView.OnItemClickListener, LoaderManager.LoaderCallbacks<Cursor>, BreadCrumbView.Controller,
	...
	private Uri getUriForFolder(long folder) {
		...
		if (folder == mRootFolder && account != null) {
			return BookmarksLoader.addAccount(BrowserContract.Bookmarks.CONTENT_URI_DEFAULT_FOLDER, account.accountType,
					account.accountName);
		}
	...
	public void onTop(BreadCrumbView view, int level, Object data) {
		...
		CursorLoader loader = (CursorLoader) ((Loader<?>) manager.getLoader(LOADER_ID_FOLDER_CONTENTS));
	...
	public void onClick(View v) {
		if (v == mButton) {
			...
			BookmarkUtils.displayRemoveBookmarkDialog(id, mTitle.getText().toString(), this, msg);
		}
	}
	 ...
	 * 
	 * @param cancel
	 *            If true, the new folder is not created. If false, the new
	...
	private LoaderCallbacks<EditBookmarkInfo> mEditInfoLoaderCallbacks = new LoaderCallbacks<EditBookmarkInfo>() {
		...
		public void onLoadFinished(Loader<EditBookmarkInfo> loader, EditBookmarkInfo info) {
			...
			if (info.lastUsedId != -1 && info.lastUsedId != info.id && !mEditingFolder) {
				if (setAccount && info.lastUsedId != mRootFolder && TextUtils.equals(info.lastUsedAccountName, info.accountName)
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		case LOADER_ID_FOLDER_CONTENTS:
			projection = new String[] { BrowserContract.Bookmarks._ID, BrowserContract.Bookmarks.TITLE, BrowserContract.Bookmarks.IS_FOLDER };
			String where = BrowserContract.Bookmarks.IS_FOLDER + " != 0";
			String whereArgs[] = null;
			if (mEditingFolder) {
				where += " AND " + BrowserContract.Bookmarks._ID + " != ?";
				whereArgs = new String[] { Long.toString(mMap.getLong(BrowserContract.Bookmarks._ID)) };
			}
			long currentFolder;
			Object data = mCrumbs.getTopData();
			if (data != null) {
			...
			return new CursorLoader(this, getUriForFolder(currentFolder), projection, where, whereArgs, BrowserContract.Bookmarks._ID
					+ " ASC");
		default:
	 ...
	 * 
	 * @param cursor
	 *            Cursor containing folders in the bookmarks database
	 * @param folderToFind
	 *            "_id" of the folder to move to.
	 * @param idIndex
	 *            Index in cursor of "_id"
	 * @throws AssertionError
	 *             if cursor is empty or there is no row with folderToFind as
	 *             its "_id".
	 */
	void moveCursorToFolder(Cursor cursor, long folderToFind, int idIndex) throws AssertionError {
		...
		if (cursor.isAfterLast()) {
			throw new AssertionError("Folder(id=" + folderToFind + ") holding this bookmark does not exist!");
		}
	}
	...
	protected void onCreate(Bundle icicle) {
		...
		mAccountAdapter = new ArrayAdapter<BookmarkAccount>(this, android.R.layout.simple_spinner_item);
	...
	private void onCurrentFolderFound() {
		...
		if (mCurrentFolder != mRootFolder) {
			...
			if (!mEditingFolder) {
				// Initially the "Bookmarks" folder should be showing, rather
				// than
				// the home screen. In the editing folder case, home screen is
				// not
		...
		if (mHandler == null) {
			mHandler = new Handler() {
				...
				public void handleMessage(Message msg) {
					...
					case SAVE_BOOKMARK:
						if (1 == msg.arg1) {
							Toast.makeText(AddBookmarkPage.this, R.string.bookmark_saved, Toast.LENGTH_LONG).show();
						} else {
							Toast.makeText(AddBookmarkPage.this, R.string.bookmark_not_saved, Toast.LENGTH_LONG).show();
						}
						break;
					case TOUCH_ICON_DOWNLOADED:
						Bundle b = msg.getData();
						sendBroadcast(BookmarkUtils.createAddToHomeIntent(AddBookmarkPage.this, b.getString(BrowserContract.Bookmarks.URL),
	...
	boolean save() {
		...
		if (mEditingExisting) {
			...
			Bitmap favicon;
			if (urlUnmodified) {
				thumbnail = (Bitmap) mMap.getParcelable(BrowserContract.Bookmarks.THUMBNAIL);
				favicon = (Bitmap) mMap.getParcelable(BrowserContract.Bookmarks.FAVICON);
			} else {
				thumbnail = null;
				favicon = null;
			}
			...
			if (mSaveToHomeScreen) {
				if (mTouchIconUrl != null && urlUnmodified) {
					Message msg = Message.obtain(mHandler, TOUCH_ICON_DOWNLOADED);
					msg.setData(bundle);
					DownloadTouchIcon icon = new DownloadTouchIcon(this, msg, mMap.getString(USER_AGENT));
					icon.execute(mTouchIconUrl);
				} else {
					sendBroadcast(BookmarkUtils.createAddToHomeIntent(this, url, title, null /* touchIcon */, favicon));
				}
			} else {
	...
	static class AccountsLoader extends CursorLoader {
		...
		static final String[] PROJECTION = new String[] { Accounts.ACCOUNT_NAME, Accounts.ACCOUNT_TYPE, Accounts.ROOT_ID, };
			...
			try {
				...
				if (checkForDupe && info.id == -1 && !TextUtils.isEmpty(url)) {
					c = cr.query(BrowserContract.Bookmarks.CONTENT_URI, new String[] { BrowserContract.Bookmarks._ID },
							BrowserContract.Bookmarks.URL + "=?", new String[] { url }, null);
				...
				if (info.id != -1) {
					c = cr.query(ContentUris.withAppendedId(BrowserContract.Bookmarks.CONTENT_URI, info.id), new String[] {
							BrowserContract.Bookmarks.PARENT, BrowserContract.Bookmarks.ACCOUNT_NAME,
							BrowserContract.Bookmarks.ACCOUNT_TYPE, BrowserContract.Bookmarks.TITLE }, null, null, null);
					if (c.moveToFirst()) {
					...
					c.close();
					c = cr.query(ContentUris.withAppendedId(BrowserContract.Bookmarks.CONTENT_URI, info.parentId),
							new String[] { BrowserContract.Bookmarks.TITLE, }, null, null, null);
					if (c.moveToFirst()) {
				...
				c = cr.query(BrowserContract.Bookmarks.CONTENT_URI, new String[] { BrowserContract.Bookmarks.PARENT, }, null, null,
						BrowserContract.Bookmarks.DATE_MODIFIED + " DESC LIMIT 1");
				if (c.moveToFirst()) {
					long parent = c.getLong(0);
					c.close();
					c = cr.query(BrowserContract.Bookmarks.CONTENT_URI, new String[] { BrowserContract.Bookmarks.TITLE,
							BrowserContract.Bookmarks.ACCOUNT_NAME, BrowserContract.Bookmarks.ACCOUNT_TYPE }, BrowserContract.Bookmarks._ID
							+ "=?", new String[] { Long.toString(parent) }, null);
					if (c.moveToFirst()) {
