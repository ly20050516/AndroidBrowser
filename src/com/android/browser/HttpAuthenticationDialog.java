...
public class HttpAuthenticationDialog {
	...
	/**
	 * Sets the listener that will be notified when the user submits the
	 * credentials.
	 */
	...
	/**
	 * Sets the listener that will be notified when the user cancels the
	...
	/**
	 * Hides, recreates, and shows the dialog. This can be used to handle
	...
	private void createDialog() {
		...
		String title = mContext.getText(R.string.sign_in_to).toString().replace("%s1", mHost).replace("%s2", mRealm);

		mDialog = new AlertDialog.Builder(mContext).setTitle(title).setIcon(android.R.drawable.ic_dialog_alert).setView(v)
	...
	/**
	 * Interface for listeners that are notified when the user submits the
	 * credentials.
	 */
	...
	/**
	 * Interface for listeners that are notified when the user cancels the
	 * dialog.
	 */
