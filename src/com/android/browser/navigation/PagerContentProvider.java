...
public class PagerContentProvider extends ContentProvider {
	...
	static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_HISTORYSITES
					+ "(_id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, url TEXT NOT NULL UNIQUE, visittime TIMESTAMP DEFAULT(datetime('now','localtime'))) ");
		}
	...
	public static interface GreenSites extends BaseColumns {
		...
		/** 全表检索 */
		public static final String SELECT_ALL = "SELECT catalog, title, url, logo, visits, isme, preset FROM " + TABLE_GREENSITES;
		
		/** 自定义检索 */
		public static final String SELECT_CUSTOM = "SELECT title, url FROM " + TABLE_GREENSITES + " WHERE preset=1 ";
	...
	public static interface HistorySites extends BaseColumns {
		...
		/** 查历史记录显示 */
		public static final String SELECT_HISTORY = "SELECT title, url FROM " + TABLE_HISTORYSITES;
	...
	public Cursor query(Uri uri, String[] columns, String selection, String[] selectionArgs, String orderBy) {
		...
		switch ( sMatcher.match(uri) ) {
			...
			case UriCode.HISTORYSITES:
				...
				if(TextUtils.isEmpty(selection)){
					...
					sql = HistorySites.SELECT_HISTORY + HistorySites.ORDER_BY_SELECT_HISTORY;
				}else{
					
					sql = HistorySites.SELECT_HISTORY + " " + selection + HistorySites.ORDER_BY_SELECT_HISTORY;
				}
				break;
		...
		Cursor c = mOpenHelper.getReadableDatabase().rawQuery(sql, selectionArgs);
