...
public class DataController {
	...
	// The standard Handler and Message classes don't allow the queue
	// manipulation
	// we want (such as peeking). So we use our own queue.
	class DataControllerHandler extends Thread {
		private BlockingQueue<DCMessage> mMessageQueue = new LinkedBlockingQueue<DCMessage>();
		...
		private void handleMessage(DCMessage msg) {
			...
			case HISTORY_UPDATE_TITLE:
				...
				break;
			case QUERY_URL_IS_BOOKMARK:
				// TODO: Look for identical messages in the queue and remove
				// them
		...
		private void doUpdateVisitedHistory(String url) {
			...
			Cursor c = null;
			try {
				c = cr.query(History.CONTENT_URI, new String[] { History._ID, History.VISITS }, History.URL + "=?", new String[] { url },
						null);
				if (c.moveToFirst()) {
					ContentValues values = new ContentValues();
		...
		private void doQueryBookmarkStatus(String url, Object replyTo) {
			...
			try {
				cursor = mContext.getContentResolver()
						.query(BookmarkUtils.getBookmarksUri(mContext), new String[] { BrowserContract.Bookmarks.URL },
