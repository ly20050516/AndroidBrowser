...
import com.android.browser.navigation.HomeView.OnSiteNavigationListener;
...
public abstract class BaseUi implements UI {
	...
	public void setActiveTab(final Tab tab) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " BaseUI setActiveTab(final Tab tab)");
		if (tab == null)
	...
	protected void attachTabToContentView(Tab tab) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " attachTabToContentView(Tab tab) ");
		if (tab == null) {

			Log.d("Liu Test", getClass().getSimpleName() + " attachTabToContentView(Tab tab) (tab == null) || (tab.getWebView() == null)");
			return;
		}
		...
		if (TextUtils.isEmpty(url)) {
			...
			if (isSearch(url)) {
				...
				if (searchEngine == null)
					return;
				SearchEngineInfo engineInfo = SearchEngines.getSearchEngineInfo(mActivity, searchEngine.getName());
				if (engineInfo == null)
					return;
				url = engineInfo.getSearchUriForQuery(url);
				// mLister.onAction can take it from here without logging
			}
			onAction(url, extra, source);
		}
	}
	...
	public void onAction(String text, String extra, String source) {
		...
		if (UrlInputView.TYPED.equals(source)) {
			...
			// return;
			// }
			mUiController.openTab(url, false, true, false);
	...
	private void removeTabFromContentView(Tab tab) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " removeTabFromContentView(Tab tab)");
		hideTitleBar();
		...
		if (mainView != null) {
			...
			Log.d("Liu Test", getClass().getSimpleName() + " removeTabFromContentView(Tab tab) remove mainview");
		...
		// needed by WebView.
		Log.d("Liu Test", getClass().getSimpleName() + " removeTabFromContentView(Tab tab) wrap.getchildcount = " + wrapper.getChildCount());
		mContentView.removeView(container);
		Log.d("Liu Test",
				getClass().getSimpleName() + " removeTabFromContentView(Tab tab) mContentView.getchildcount = "
						+ mContentView.getChildCount());
	...
	public void onSetWebView(Tab tab, WebView webView) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " BaseUI onSetWebView");
		View container = tab.getViewContainer();
		if (container == null) {
		...
		Log.d("Liu Test", getClass().getSimpleName() + " BaseUI onSetWebView webView = " + webView);
		if (webView == null) {
	 ...
	 * @param subView
	 *            webview to be set as a subwindow for the tab
	 */
	@Override
	public void createSubWindow(Tab tab, WebView subView) {
		...
		cancel.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				WebViewClassic.fromWebView(cancelSubView).getWebChromeClient().onCloseWindow(cancelSubView);
			}
		});
	...
	boolean canShowTitleBar() {
		return !isTitleBarShowing() && !isActivityPaused() && (getActiveTab() != null) && (getWebView() != null)
	...
	private void updateLockIconImage(SecurityState securityState) {
		...
		if (securityState == SecurityState.SECURITY_STATE_SECURE) {
			...
			// TODO: It would be good to have different icons for insecure vs
			// mixed content.
			// See http://b/5403800
			d = mLockIconMixed;
		}
	...
	public void setShouldShowErrorConsole(Tab tab, boolean flag) {
		...
		if (flag) {
			...
			mErrorConsoleContainer.addView(errorConsole, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
	...
	public void showMaxTabsWarning() {
		Toast warning = Toast.makeText(mActivity, mActivity.getString(R.string.max_tabs_warning), Toast.LENGTH_SHORT);
		warning.show();
	}
	...
	class HomeViewOnSiteNavigationListener implements OnSiteNavigationListener {
		...
		public void onSiteNavigation(View v) {
			...
			if (baseObject != null) {
				finishInput(baseObject.getUrl(), null, "browser-type");
			}
		}
	}
}
