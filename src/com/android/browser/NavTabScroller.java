	...
	protected void onPull(int delta) {
		...
		if (delta == 0 && mPullValue != 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (child == null)
					break;
				ObjectAnimator trans = ObjectAnimator.ofFloat(child, mHorizontal ? "translationX" : "translationY",
						mHorizontal ? getTranslationX() : getTranslationY(), 0);
				ObjectAnimator rot = ObjectAnimator.ofFloat(child, mHorizontal ? "rotationY" : "rotationX", mHorizontal ? getRotationY()
						: getRotationX(), 0);
