...
/**
 * Manages the interaction between the secure system setting for default
 * geolocation permissions and the browser.
 */
class SystemAllowGeolocationOrigins {

	// Preference key for the value of the system setting last read by the
	// browser
	private final static String LAST_READ_ALLOW_GEOLOCATION_ORIGINS = "last_read_allow_geolocation_origins";
	...
	/**
	 * Checks whether the setting has changed and installs an observer to listen
	 * for future changes. Must be called on the application main thread.
	 */
	public void start() {
	...
	/**
	 * Checks to see if the system setting has changed and if so, updates the
	 * Geolocation permissions accordingly.
	 */
	 ...
	 * 
	 * @param setting
	 *            A space-separated list of origins.
	...
	/**
	 * Gets the difference between two sets. Does not modify any of the
	 * arguments.
	 * 
	 * @return A set containing all elements in {@code x} that are not in
	 *         {@code y}.
	 */
	...
	private String getSystemSetting() {
		String value = Settings.Secure.getString(mContext.getContentResolver(), Settings.Secure.ALLOWED_GEOLOCATION_ORIGINS);
	...
	/**
	 * Removes geolocation permissions for the given origins, if they are
	 * allowed. If they are denied or not set, nothing is done.
	 */
