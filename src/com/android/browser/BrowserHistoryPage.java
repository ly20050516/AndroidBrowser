 ...
 */
public class BrowserHistoryPage extends Fragment implements LoaderCallbacks<Cursor>, OnChildClickListener {
	...
	static interface HistoryQuery {
		static final String[] PROJECTION = new String[] { Combined._ID, // 0
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		case LOADER_MOST_VISITED: {
			Uri uri = combinedBuilder.appendQueryParameter(BrowserContract.PARAM_LIMIT, mMostVisitsLimit).build();
			String where = Combined.VISITS + " > 0";
			CursorLoader loader = new CursorLoader(getActivity(), uri, HistoryQuery.PROJECTION, where, null, Combined.VISITS + " DESC");
			return loader;
		}
	...
	void selectGroup(int position) {
		mGroupItemClickListener.onItemClick(null, mAdapter.getGroupView(position, false, null, null), position, position);
	}
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
		...
		case LOADER_HISTORY: {
			...
			if (!mAdapter.isEmpty() && mGroupList != null && mGroupList.getCheckedItemPosition() == ListView.INVALID_POSITION) {
				selectGroup(0);
			}
	...
	void promptToClearHistory() {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity()).setMessage(R.string.pref_privacy_clear_history_dlg)
				.setIcon(android.R.drawable.ic_dialog_alert).setNegativeButton(R.string.cancel, null)
	...
	private class HistoryAdapter extends DateSortedExpandableListAdapter {
		...
		private boolean isMostVisitedEmpty() {
			return mMostVisited == null || mMostVisited.isClosed() || mMostVisited.getCount() == 0;
		}
