...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
	...
	private static final String[] PROJECTION = new String[] { BrowserContract.Bookmarks._ID, BrowserContract.Bookmarks.TITLE,
			BrowserContract.Bookmarks.URL, BrowserContract.Bookmarks.FAVICON, BrowserContract.Bookmarks.IS_FOLDER,
			BrowserContract.Bookmarks.POSITION, /* needed for order by */
			BrowserContract.Bookmarks.THUMBNAIL, BrowserContract.Bookmarks.PARENT };
	...
	static SharedPreferences getWidgetState(Context context, int widgetId) {
		return context.getSharedPreferences(String.format("widgetState-%d", widgetId), Context.MODE_PRIVATE);
	}

	static void deleteWidgetState(Context context, int widgetId) {
		File file = context.getSharedPrefsFile(String.format("widgetState-%d", widgetId));
		if (file.exists()) {
			if (!file.delete()) {
				file.deleteOnExit();
			}
	...
	static void setupWidgetState(Context context, int widgetId, long rootFolder) {
		...
		pref.edit().putLong(STATE_CURRENT_FOLDER, rootFolder).putLong(STATE_ROOT_FOLDER, rootFolder).apply();
	}
	...
	static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
		...
		void saveState() {
			...
			mPreferences.edit().putLong(STATE_CURRENT_FOLDER, mCurrentFolder).putLong(STATE_ROOT_FOLDER, mRootFolder).commit();
		}
		...
		public RemoteViews getLoadingView() {
			return new RemoteViews(mContext.getPackageName(), R.layout.bookmarkthumbnailwidget_item);
		}
		...
		public RemoteViews getViewAt(int position) {
			...
			if (isFolder) {
				views = new RemoteViews(mContext.getPackageName(), R.layout.bookmarkthumbnailwidget_item_folder);
			} else {
				views = new RemoteViews(mContext.getPackageName(), R.layout.bookmarkthumbnailwidget_item);
			}
			...
			if (isFolder) {
				...
				if (blob != null && blob.length > 0) {
					thumbnail = BitmapFactory.decodeByteArray(blob, 0, blob.length, options);
				...
				if (blob != null && blob.length > 0) {
					favicon = BitmapFactory.decodeByteArray(blob, 0, blob.length, options);
			...
			Intent fillin;
			if (isFolder) {
				fillin = new Intent(ACTION_CHANGE_FOLDER).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mWidgetId).putExtra(Bookmarks._ID,
						id);
			} else {
				if (!TextUtils.isEmpty(url)) {
					fillin = new Intent(Intent.ACTION_VIEW).addCategory(Intent.CATEGORY_BROWSABLE).setData(Uri.parse(url));
				} else {
		...
		void loadBookmarks() {
			...
			Uri uri = ContentUris.withAppendedId(BrowserContract.Bookmarks.CONTENT_URI_DEFAULT_FOLDER, mCurrentFolder);
			mBookmarks = mContext.getContentResolver().query(uri, PROJECTION, null, null, null);
			if (mCurrentFolder != mRootFolder) {
				uri = ContentUris.withAppendedId(BrowserContract.Bookmarks.CONTENT_URI, mCurrentFolder);
