...
public class PreloadedTabControl {
	...
	private void maybeSetQuery(final String query, SearchBox sb) {
		if (!TextUtils.equals(mLastQuery, query)) {
			if (sb != null) {
				if (LOGD_ENABLED)
					Log.d(LOGTAG, "Changing searchbox query to " + query);
				sb.setVerbatim(true);
				sb.setQuery(query);
				sb.onchange(new SearchBox.SearchBoxListener() {
					...
					public void onChangeComplete(boolean called) {
						if (mDestroyed)
							return;
						if (LOGD_ENABLED)
							Log.d(LOGTAG, "Changed searchbox query: " + called);
						if (called) {
							mLastQuery = query;
						}
					}
	...
	public boolean searchBoxSubmit(final String query, final String fallbackUrl, final Map<String, String> fallbackHeaders) {
		...
		if (LOGD_ENABLED)
			Log.d(LOGTAG, "Submitting query " + query);
		...
		sb.onsubmit(new SearchBox.SearchBoxListener() {
			...
			public void onSubmitComplete(boolean called) {
				if (mDestroyed)
					return;
				if (LOGD_ENABLED)
					Log.d(LOGTAG, "Query submitted: " + called);
				if (!called) {
					if (LOGD_ENABLED)
						Log.d(LOGTAG, "Query not submitted; falling back");
					loadUrl(fallbackUrl, fallbackHeaders);
					// make sure that the failed, preloaded URL is cleared from
					// the back stack
					mTab.clearBackStackWhenItemAdded(Pattern.compile("^" + Pattern.quote(fallbackUrl) + "$"));
				} else {
					// ignore the next fragment change, to avoid leaving a blank
					// page in the browser
					// after the query has been submitted.
					String currentWithoutFragment = Uri.parse(currentUrl).buildUpon().fragment(null).toString();
					mTab.clearBackStackWhenItemAdded(Pattern.compile("^" + Pattern.quote(currentWithoutFragment) + "(\\#.*)?" + "$"));
				}
			}
		});
		return true;
		...
		if (sb != null) {
			...
			sb.oncancel(new SearchBox.SearchBoxListener() {
				...
				public void onCancelComplete(boolean called) {
					if (LOGD_ENABLED)
						Log.d(LOGTAG, "Query cancelled: " + called);
				}
			});
		}
	}
