...
public class BrowserProvider2 extends SQLiteContentProvider {
	...
	static final String TABLE_BOOKMARKS_JOIN_IMAGES = "bookmarks LEFT OUTER JOIN images " + "ON bookmarks.url = images." + Images.URL;
	static final String TABLE_HISTORY_JOIN_IMAGES = "history LEFT OUTER JOIN images " + "ON history.url = images." + Images.URL;
	...
	static final String FORMAT_COMBINED_JOIN_SUBQUERY_JOIN_IMAGES = "history LEFT OUTER JOIN (%s) bookmarks "
			+ "ON history.url = bookmarks.url LEFT OUTER JOIN images " + "ON history.url = images.url_key";

	static final String DEFAULT_SORT_HISTORY = History.DATE_LAST_VISITED + " DESC";
	static final String DEFAULT_SORT_ACCOUNTS = Accounts.ACCOUNT_NAME + " IS NOT NULL DESC, " + Accounts.ACCOUNT_NAME + " ASC";
	...
	private static final String[] SUGGEST_PROJECTION = new String[] {
			...
			bookmarkOrHistoryLiteral(Combined.URL, Integer.toString(R.drawable.ic_bookmark_off_holo_dark),
					Integer.toString(R.drawable.ic_history_holo_dark)), qualifyColumn(TABLE_HISTORY, History.DATE_LAST_VISITED) };

	private static final String SUGGEST_SELECTION = "history.url LIKE ? OR history.url LIKE ? OR history.url LIKE ? OR history.url LIKE ?"
			+ " OR history.title LIKE ? OR bookmarks.title LIKE ?";

	private static final String ZERO_QUERY_SUGGEST_SELECTION = TABLE_HISTORY + "." + History.DATE_LAST_VISITED + " != 0";

	private static final String IMAGE_PRUNE = "url_key NOT IN (SELECT url FROM bookmarks "
			+ "WHERE url IS NOT NULL AND deleted == 0) AND url_key NOT IN " + "(SELECT url FROM history WHERE url IS NOT NULL)";
	...
	static final String DEFAULT_BOOKMARKS_SORT_ORDER = Bookmarks.IS_FOLDER + " DESC, position ASC, _id ASC";
	...
	static {
		...
		map.put(Bookmarks.PARENT_SOURCE_ID, "(SELECT " + Bookmarks.SOURCE_ID + " FROM " + TABLE_BOOKMARKS + " A WHERE " + "A."
				+ Bookmarks._ID + "=" + TABLE_BOOKMARKS + "." + Bookmarks.PARENT + ") AS " + Bookmarks.PARENT_SOURCE_ID);
		map.put(Bookmarks.INSERT_AFTER_SOURCE_ID, "(SELECT " + Bookmarks.SOURCE_ID + " FROM " + TABLE_BOOKMARKS + " A WHERE " + "A."
				+ Bookmarks._ID + "=" + TABLE_BOOKMARKS + "." + Bookmarks.INSERT_AFTER + ") AS " + Bookmarks.INSERT_AFTER_SOURCE_ID);
		map.put(Bookmarks.TYPE, "CASE " + " WHEN " + Bookmarks.IS_FOLDER + "=0 THEN " + Bookmarks.BOOKMARK_TYPE_BOOKMARK + " WHEN "
				+ ChromeSyncColumns.SERVER_UNIQUE + "='" + ChromeSyncColumns.FOLDER_NAME_BOOKMARKS_BAR + "' THEN "
				+ Bookmarks.BOOKMARK_TYPE_BOOKMARK_BAR_FOLDER + " WHEN " + ChromeSyncColumns.SERVER_UNIQUE + "='"
				+ ChromeSyncColumns.FOLDER_NAME_OTHER_BOOKMARKS + "' THEN " + Bookmarks.BOOKMARK_TYPE_OTHER_FOLDER + " ELSE "
				+ Bookmarks.BOOKMARK_TYPE_FOLDER + " END AS " + Bookmarks.TYPE);

		// Other bookmarks
		OTHER_BOOKMARKS_PROJECTION_MAP.putAll(BOOKMARKS_PROJECTION_MAP);
		OTHER_BOOKMARKS_PROJECTION_MAP.put(Bookmarks.POSITION, Long.toString(Long.MAX_VALUE) + " AS " + Bookmarks.POSITION);
		...
		map.put(Combined.IS_BOOKMARK, "CASE WHEN " + TABLE_BOOKMARKS + "." + Bookmarks._ID + " IS NOT NULL THEN 1 ELSE 0 END AS "
				+ Combined.IS_BOOKMARK);
	...
	static final String bookmarkOrHistoryColumn(String column) {
		return "CASE WHEN bookmarks." + column + " IS NOT NULL THEN " + "bookmarks." + column + " ELSE history." + column + " END AS "
				+ column;
	}
	...
	final class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + TABLE_BOOKMARKS + "(" + Bookmarks._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + Bookmarks.TITLE
					+ " TEXT," + Bookmarks.URL + " TEXT," + Bookmarks.IS_FOLDER + " INTEGER NOT NULL DEFAULT 0," + Bookmarks.PARENT
					+ " INTEGER," + Bookmarks.POSITION + " INTEGER NOT NULL," + Bookmarks.INSERT_AFTER + " INTEGER," + Bookmarks.IS_DELETED
					+ " INTEGER NOT NULL DEFAULT 0," + Bookmarks.ACCOUNT_NAME + " TEXT," + Bookmarks.ACCOUNT_TYPE + " TEXT,"
					+ Bookmarks.SOURCE_ID + " TEXT," + Bookmarks.VERSION + " INTEGER NOT NULL DEFAULT 1," + Bookmarks.DATE_CREATED
					+ " INTEGER," + Bookmarks.DATE_MODIFIED + " INTEGER," + Bookmarks.DIRTY + " INTEGER NOT NULL DEFAULT 0,"
					+ Bookmarks.SYNC1 + " TEXT," + Bookmarks.SYNC2 + " TEXT," + Bookmarks.SYNC3 + " TEXT," + Bookmarks.SYNC4 + " TEXT,"
					+ Bookmarks.SYNC5 + " TEXT" + ");");

			// TODO indices

			db.execSQL("CREATE TABLE " + TABLE_HISTORY + "(" + History._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + History.TITLE
					+ " TEXT," + History.URL + " TEXT NOT NULL," + History.DATE_CREATED + " INTEGER," + History.DATE_LAST_VISITED
					+ " INTEGER," + History.VISITS + " INTEGER NOT NULL DEFAULT 0," + History.USER_ENTERED + " INTEGER" + ");");

			db.execSQL("CREATE TABLE " + TABLE_IMAGES + " (" + Images.URL + " TEXT UNIQUE NOT NULL," + Images.FAVICON + " BLOB,"
					+ Images.THUMBNAIL + " BLOB," + Images.TOUCH_ICON + " BLOB" + ");");
			db.execSQL("CREATE INDEX imagesUrlIndex ON " + TABLE_IMAGES + "(" + Images.URL + ")");

			db.execSQL("CREATE TABLE " + TABLE_SEARCHES + " (" + Searches._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + Searches.SEARCH
					+ " TEXT," + Searches.DATE + " LONG" + ");");
		...
		void createThumbnails(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_THUMBNAILS + " (" + Thumbnails._ID + " INTEGER PRIMARY KEY,"
			...
			if (oldDbFile.exists()) {
				...
				try {
					...
					c = oldDb.query(table, new String[] { BookmarkColumns.URL, // 0
							...
							BookmarkColumns.CREATED, // 4
					}, BookmarkColumns.VISITS + " > 0 OR " + BookmarkColumns.BOOKMARK + " = 0", null, null, null, null);
					if (c != null) {
						while (c.moveToNext()) {
							ContentValues values = new ContentValues();
							String url = c.getString(0);
		...
		void createAccountsView(SQLiteDatabase db) {
			db.execSQL("CREATE VIEW IF NOT EXISTS v_accounts AS " + "SELECT NULL AS " + Accounts.ACCOUNT_NAME + ", NULL AS "
					+ Accounts.ACCOUNT_TYPE + ", " + FIXED_ID_ROOT + " AS " + Accounts.ROOT_ID + " UNION ALL SELECT "
					+ Accounts.ACCOUNT_NAME + ", " + Accounts.ACCOUNT_TYPE + ", " + Bookmarks._ID + " AS " + Accounts.ROOT_ID + " FROM "
					+ TABLE_BOOKMARKS + " WHERE " + ChromeSyncColumns.SERVER_UNIQUE + " = \"" + ChromeSyncColumns.FOLDER_NAME_BOOKMARKS_BAR
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion < 25) {
				...
				mSyncHelper.onAccountsChanged(db, new Account[] {}); // remove
																		// all
																		// sync
																		// info
				onCreate(db);
		...
		private void addDefaultBookmarks(SQLiteDatabase db, long parentId) {
			...
			try {
				...
				for (int i = 0; i < size; i = i + 2) {
					...
					db.execSQL("INSERT INTO bookmarks (" + Bookmarks.TITLE + ", " + Bookmarks.URL + ", " + Bookmarks.IS_FOLDER + ","
							+ Bookmarks.PARENT + "," + Bookmarks.POSITION + "," + Bookmarks.DATE_CREATED + ") VALUES (" + "'"
							+ bookmarks[i] + "', " + "'" + bookmarkDestination + "', " + "0," + parent + "," + Integer.toString(i) + ","
							+ now + ");");
		...
		// XXX: This is a major hack to remove our dependency on gsf constants
		// and
	...
	Object[] getSelectionWithAccounts(Uri uri, String selection, String[] selectionArgs) {
		...
		if (accountType != null && accountName != null) {
			if (!isNullAccount(accountType) && !isNullAccount(accountName)) {
				...
				selection = DatabaseUtils.concatenateWhere(selection, Bookmarks.ACCOUNT_NAME + " IS NULL AND " + Bookmarks.ACCOUNT_TYPE
						+ " IS NULL");
			}
		}
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		case BOOKMARKS_FOLDER: {
			...
			if (!useAccount) {
				...
				qb.setProjectionMap(BOOKMARKS_PROJECTION_MAP);
				String where = Bookmarks.ACCOUNT_TYPE + "=? AND " + Bookmarks.ACCOUNT_NAME + "=? " + "AND parent = " + "(SELECT _id FROM "
						+ TABLE_BOOKMARKS + " WHERE " + ChromeSyncColumns.SERVER_UNIQUE + "=" + "'"
						+ ChromeSyncColumns.FOLDER_NAME_BOOKMARKS_BAR + "' " + "AND account_type = ? AND account_name = ?) " + "AND "
				...
				args = DatabaseUtils.appendSelectionArgs(args, new String[] { accountType, accountName,
		...
		case SYNCSTATE_ID: {
			...
			String selectionWithId = (SyncStateContract.Columns._ID + "=" + ContentUris.parseId(uri) + " ")
	...
	private Cursor doSuggestQuery(String selection, String[] selectionArgs, String limit) {
		if (TextUtils.isEmpty(selectionArgs[0])) {
			...
			selection = DatabaseUtils.concatenateWhere(selection, Bookmarks.IS_DELETED + "=0 AND " + Bookmarks.IS_FOLDER + "=0");

		}
		Cursor c = mOpenHelper.getReadableDatabase().query(TABLE_BOOKMARKS_JOIN_HISTORY, SUGGEST_PROJECTION, selection, selectionArgs,
				null, null, null, null);
	...
	private String[] createCombinedQuery(Uri uri, String[] projection, SQLiteQueryBuilder qb) {
		...
		if (selection != null) {
			...
			if (selectionArgs != null) {
				// We use the selection twice, hence we need to duplicate the
				// args
		...
		where += String.format(" AND %s NOT IN (SELECT %s FROM %s)", Combined.URL, History.URL, TABLE_HISTORY);
		String bookmarksSubQuery = qb.buildQuery(null, where, null, null, null, null);
		// Put it all together
	...
	public int deleteInTransaction(Uri uri, String selection, String[] selectionArgs, boolean callerIsSyncAdapter) {
		...
		case LEGACY: {
			String[] projection = new String[] { Combined._ID, Combined.IS_BOOKMARK, Combined.URL };
			SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
			String[] args = createCombinedQuery(uri, projection, qb);
			if (selectionArgs == null) {
				selectionArgs = args;
			...
			while (c.moveToNext()) {
				...
				if (isBookmark) {
					deleted += deleteBookmarks(Bookmarks._ID + "=?", new String[] { Long.toString(id) }, callerIsSyncAdapter);
					db.delete(TABLE_HISTORY, History.URL + "=?", new String[] { url });
				} else {
					deleted += db.delete(TABLE_HISTORY, Bookmarks._ID + "=?", new String[] { Long.toString(id) });
				}
			}
			c.close();
			break;
	...
	long queryDefaultFolderId(String accountName, String accountType) {
		if (!isNullAccount(accountName) && !isNullAccount(accountType)) {
			...
			Cursor c = db.query(TABLE_BOOKMARKS, new String[] { Bookmarks._ID }, ChromeSyncColumns.SERVER_UNIQUE + " = ?"
					+ " AND account_type = ? AND account_name = ?", new String[] { ChromeSyncColumns.FOLDER_NAME_BOOKMARKS_BAR,
	...
	public Uri insertInTransaction(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
		...
		case BOOKMARKS: {
			...
			// If no position is requested put the bookmark at the beginning of
			// the list
			if (!values.containsKey(Bookmarks.POSITION)) {
				values.put(Bookmarks.POSITION, Long.toString(Long.MIN_VALUE));
			}

			// Extract out the image values so they can be inserted into the
			// images table
		...
		case HISTORY: {
			...
			// Extract out the image values so they can be inserted into the
			// images table
			ContentValues imageValues = extractImageValues(values, values.getAsString(History.URL));
			if (imageValues != null) {
				db.insertOrThrow(TABLE_IMAGES, Images.FAVICON, imageValues);
			}
	...
	private boolean isValidParent(String accountType, String accountName, long parentId) {
		String[] parent = getAccountNameAndType(parentId);
		if (parent != null && TextUtils.equals(accountName, parent[0]) && TextUtils.equals(accountType, parent[1])) {
			return true;
		}
		return false;
	}
	...
	private String filterSearchClient(String url) {
		...
		if (index > 0 && url.contains(".google.")) {
			...
			if (end > 0) {
				url = url.substring(0, index).concat(url.substring(end + 1));
			} else {
	...
	/**
	 * Searches are unique, so perform an UPSERT manually since SQLite doesn't
	 * support them.
	 */
	private long insertSearchesInTransaction(SQLiteDatabase db, ContentValues values) {
		...
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_SEARCHES, new String[] { Searches._ID }, Searches.SEARCH + "=?", new String[] { search }, null, null,
					null);
			if (cursor.moveToNext()) {
				long id = cursor.getLong(0);
				db.update(TABLE_SEARCHES, values, Searches._ID + "=?", new String[] { Long.toString(id) });
				return id;
			} else {
	...
	/**
	 * Settings are unique, so perform an UPSERT manually since SQLite doesn't
	 * support them.
	 */
	private long insertSettingsInTransaction(SQLiteDatabase db, ContentValues values) {
		...
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_SETTINGS, new String[] { Settings.KEY }, Settings.KEY + "=?", keyArray, null, null, null);
			if (cursor.moveToNext()) {
				long id = cursor.getLong(0);
	...
	public int updateInTransaction(Uri uri, ContentValues values, String selection, String[] selectionArgs, boolean callerIsSyncAdapter) {
		...
		case BOOKMARKS: {
			...
			selection = (String) withAccount[0];
			selectionArgs = (String[]) withAccount[1];
			modified = updateBookmarksInTransaction(values, selection, selectionArgs, callerIsSyncAdapter);
			if (modified > 0) {
				refreshWidgets();
			}
			break;
		...
		case SYNCSTATE: {
			modified = mSyncHelper.update(mDb, values, appendAccountToSelection(uri, selection), selectionArgs);
			break;
		}
		...
		case IMAGES: {
			...
			int count = db.update(TABLE_IMAGES, values, Images.URL + "=?", new String[] { url });
			if (count == 0) {
		...
		case THUMBNAILS: {
			modified = db.update(TABLE_THUMBNAILS, values, selection, selectionArgs);
			break;
		}
	...
	// Thus, we check to see if the images we are about to store are already
	// there
	...
	private boolean shouldUpdateImages(SQLiteDatabase db, String url, ContentValues values) {
		final String[] projection = new String[] { Images.FAVICON, Images.THUMBNAIL, Images.TOUCH_ICON, };
		Cursor cursor = db.query(TABLE_IMAGES, projection, Images.URL + "=?", new String[] { url }, null, null, null);
	...
	/**
	 * Does a query to find the matching bookmarks and updates each one with the
	 * provided values.
	 */
	int updateBookmarksInTransaction(ContentValues values, String selection, String[] selectionArgs, boolean callerIsSyncAdapter) {
		int count = 0;
		final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		final String[] bookmarksProjection = new String[] { Bookmarks._ID, // 0
				Bookmarks.VERSION, // 1
				Bookmarks.URL, // 2
				Bookmarks.TITLE, // 3
		...
		if (updatingParent) {
			...
			Cursor c = db.query(TABLE_BOOKMARKS, new String[] { Bookmarks.ACCOUNT_NAME, Bookmarks.ACCOUNT_TYPE }, "_id = ?",
					new String[] { Long.toString(parent) }, null, null, null);
			if (c.moveToFirst()) {
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				if (updatingParent
						&& (!TextUtils.equals(accountName, parentAccountName) || !TextUtils.equals(accountType, parentAccountType))) {
					...
					if (cursor.getInt(4) != 0) {
						...
						count += updateBookmarksInTransaction(updateChildren, Bookmarks.PARENT + "=?", new String[] { Long.toString(id) },
								callerIsSyncAdapter);
					}
	...
	int updateHistoryInTransaction(ContentValues values, String selection, String[] selectionArgs) {
		...
		final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		filterSearchClient(selectionArgs);
		Cursor cursor = query(History.CONTENT_URI, new String[] { History._ID, History.URL }, selection, selectionArgs, null);
		try {
			String[] args = new String[1];
	...
	String appendAccountToSelection(Uri uri, String selection) {
		...
		if (validAccount) {
			StringBuilder selectionSb = new StringBuilder(RawContacts.ACCOUNT_NAME + "=" + DatabaseUtils.sqlEscapeString(accountName)
					+ " AND " + RawContacts.ACCOUNT_TYPE + "=" + DatabaseUtils.sqlEscapeString(accountType));
	...
	boolean shouldNotifyLegacy(Uri uri) {
		if (uri.getPathSegments().contains("history") || uri.getPathSegments().contains("bookmarks")
	...
	protected boolean syncToNetwork(Uri uri) {
		if (BrowserContract.AUTHORITY.equals(uri.getAuthority()) && uri.getPathSegments().contains("bookmarks")) {
	...
	static class SuggestionsCursor extends AbstractCursor {
		...
		private static final String[] COLUMNS = new String[] { BaseColumns._ID, SearchManager.SUGGEST_COLUMN_INTENT_ACTION,
				SearchManager.SUGGEST_COLUMN_INTENT_DATA, SearchManager.SUGGEST_COLUMN_TEXT_1, SearchManager.SUGGEST_COLUMN_TEXT_2,
				SearchManager.SUGGEST_COLUMN_TEXT_2_URL, SearchManager.SUGGEST_COLUMN_ICON_1, SearchManager.SUGGEST_COLUMN_LAST_ACCESS_HINT };
	...
	private static final String SQL_CREATE_VIEW_OMNIBOX_SUGGESTIONS = "CREATE VIEW IF NOT EXISTS v_omnibox_suggestions " + " AS "
			+ "  SELECT _id, url, title, 1 AS bookmark, 0 AS visits, 0 AS date" + "  FROM bookmarks "
			+ "  WHERE deleted = 0 AND folder = 0 " + "  UNION ALL " + "  SELECT _id, url, title, 0 AS bookmark, visits, date "
			+ "  FROM history " + "  WHERE url NOT IN (SELECT url FROM bookmarks" + "    WHERE deleted = 0 AND folder = 0) "
			+ "  ORDER BY bookmark DESC, visits DESC, date DESC ";

	private static final String SQL_WHERE_ACCOUNT_HAS_BOOKMARKS = "0 < ( " + "SELECT count(*) " + "FROM bookmarks "
			+ "WHERE deleted = 0 AND folder = 0 " + "  AND ( " + "    v_accounts.account_name = bookmarks.account_name "
			+ "    OR (v_accounts.account_name IS NULL AND bookmarks.account_name IS NULL) " + "  ) " + "  AND ( "
