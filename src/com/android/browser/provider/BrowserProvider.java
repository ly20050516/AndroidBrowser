...
public class BrowserProvider extends ContentProvider {
	...
	static final String[] TABLE_NAMES = new String[] { "bookmarks", "searches" };
	private static final String[] SUGGEST_PROJECTION = new String[] { "_id", "url", "title", "bookmark", "user_entered" };
	private static final String SUGGEST_SELECTION = "(url LIKE ? OR url LIKE ? OR url LIKE ? OR url LIKE ?"
	...
	private static final String[] COLUMNS = new String[] { "_id", SearchManager.SUGGEST_COLUMN_INTENT_ACTION,
			SearchManager.SUGGEST_COLUMN_INTENT_DATA, SearchManager.SUGGEST_COLUMN_TEXT_1, SearchManager.SUGGEST_COLUMN_TEXT_2,
			SearchManager.SUGGEST_COLUMN_TEXT_2_URL, SearchManager.SUGGEST_COLUMN_ICON_1, SearchManager.SUGGEST_COLUMN_ICON_2,
	...
	static {
		...
		URI_MATCHER.addURI("browser", TABLE_NAMES[URI_MATCH_BOOKMARKS] + "/" + SearchManager.SUGGEST_URI_PATH_QUERY,
	...
	public static String getClientId(ContentResolver cr) {
		...
		// include prefix
		try {
			searchClientIdCursor = cr.query(Uri.parse("content://com.google.settings/partner"), new String[] { "value" },
			...
			if (searchClientIdCursor != null && searchClientIdCursor.moveToNext()) {
				...
				legacyClientIdCursor = cr.query(Uri.parse("content://com.google.settings/partner"), new String[] { "value" },
	...
	static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE bookmarks (" + "_id INTEGER PRIMARY KEY," + "title TEXT," + "url TEXT NOT NULL," + "visits INTEGER,"
					+ "date LONG," + "created LONG," + "description TEXT," + "bookmark INTEGER," + "favicon BLOB DEFAULT NULL,"
					+ "thumbnail BLOB DEFAULT NULL," + "touch_icon BLOB DEFAULT NULL," + "user_entered INTEGER" + ");");
			...
			try {
				for (int i = 0; i < size; i = i + 2) {
					...
					db.execSQL("INSERT INTO bookmarks (title, url, visits, " + "date, created, bookmark)" + " VALUES('" + bookmarks[i]
							+ "', '" + bookmarkDestination + "', 0, 0, 0, 1);");
				}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion < 24) {
				...
				db.execSQL("CREATE TABLE bookmarks (" + "_id INTEGER PRIMARY KEY," + "title TEXT," + "url TEXT NOT NULL,"
						+ "visits INTEGER," + "date LONG," + "created LONG," + "description TEXT," + "bookmark INTEGER,"
						+ "favicon BLOB DEFAULT NULL," + "thumbnail BLOB DEFAULT NULL," + "touch_icon BLOB DEFAULT NULL,"
		...
		private void removeGears() {
			new Thread() {
				...
				public void run() {
					...
					File appPluginsDir = new File(browserDataDirString + File.separator + appPluginsDirString);
					...
					File gearsDataDir = new File(browserDataDirString + File.separator + gearsPrefix);
	...
	private void fixPicasaBookmark() {
		...
		try {
			if (!cursor.moveToFirst()) {
				...
				db.execSQL("INSERT INTO bookmarks (title, url, visits, " + "date, created, bookmark)" + " VALUES('"
						+ getContext().getString(R.string.picasa) + "', '" + PICASA_URL + "', 0, 0, " + new Date().getTime() + ", 1);");
			}
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	private class MySuggestionCursor extends AbstractCursor {
		...
		public MySuggestionCursor(Cursor hc, Cursor sc, String string) {
			...
			if (mSuggestCursor == null) {
				...
				mSuggestText1Id = mSuggestCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_1);
				mSuggestText2Id = mSuggestCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2);
				mSuggestText2UrlId = mSuggestCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2_URL);
				mSuggestQueryId = mSuggestCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_QUERY);
				mSuggestIntentExtraDataId = mSuggestCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA);
			}
		}
			...
			if ((mPos != -1 && mHistoryCursor != null)) {
				...
				case SUGGEST_COLUMN_QUERY_ID:
					if (type == 0) {
						...
						// Return the url in the intent query column. This is
						// ignored
						// within the browser because our searchable is set to
						// android:searchMode="queryRewriteFromData", but it is
						// used by
	...
	public Cursor query(Uri url, String[] projectionIn, String selection, String[] selectionArgs, String sortOrder)
		...
		Cursor c = mOpenHelper.getReadableDatabase().query(TABLE_NAMES[match % 10], projection,
				DatabaseUtils.concatenateWhere(whereClause, selection), selectionArgs, null, null, sortOrder, null);
	...
	private Cursor doSuggestQuery(String selection, String[] selectionArgs, boolean bookmarksOnly) {
		...
		Cursor c = mOpenHelper.getReadableDatabase().query(TABLE_NAMES[URI_MATCH_BOOKMARKS], SUGGEST_PROJECTION, suggestSelection, myArgs,
				null, null, ORDER_BY, Integer.toString(mMaxSuggestionLongSize));
	...
	public Uri insert(Uri url, ContentValues initialValues) {
		...
		if (isBookmarkTable && initialValues.containsKey(BookmarkColumns.BOOKMARK)
	...
	public int delete(Uri url, String where, String[] whereArgs) {
		...
		if (isBookmarkTable) {
			Cursor cursor = cr.query(Browser.BOOKMARKS_URI, new String[] { BookmarkColumns.BOOKMARK }, "_id = " + id, null, null);
	...
	public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
		...
		if (match == URI_MATCH_BOOKMARKS_ID || match == URI_MATCH_BOOKMARKS) {
			...
			if (values.containsKey(BookmarkColumns.BOOKMARK)) {
			...
			} else if ((values.containsKey(BookmarkColumns.TITLE) || values.containsKey(BookmarkColumns.URL))
				...
				// so use it.
				Cursor cursor = cr.query(Browser.BOOKMARKS_URI, new String[] { BookmarkColumns.BOOKMARK }, BookmarkColumns._ID + " = "
	 ...
	 * 
	 * TODO: Put this in TextUtils to be used by other packages doing something
	 * similar.
	 * 
	 * @param url
	 *            a url to strip, like "http://www.google.com/"
	...
	public static Cursor getBookmarksSuggestions(ContentResolver cr, String constraint) {
		...
		return cr.query(uri, SUGGEST_PROJECTION, SUGGEST_SELECTION, new String[] { constraint }, ORDER_BY);
	}
