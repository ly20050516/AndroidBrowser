...
public class PageDialogsHandler {
	...
	public void onConfigurationChanged(Configuration config) {
		if (mPageInfoDialog != null) {
			...
			showPageInfo(mPageInfoView, mPageInfoFromShowSSLCertificateOnError, mUrlCertificateOnError);
		}
		...
		if (mSSLCertificateOnErrorDialog != null) {
			...
			showSSLCertificateOnError(mSSLCertificateOnErrorView, mSSLCertificateOnErrorHandler, mSSLCertificateOnErrorError);
		}
	 ...
	 *            or simply dismiss.
	 * @param urlCertificateOnError
	 *            The URL that invokes SSLCertificateError. Null when
	 *            fromShowSSLCertificateOnError is false.
	 */
	void showPageInfo(final Tab tab, final boolean fromShowSSLCertificateOnError, final String urlCertificateOnError) {
		if (tab == null)
			return;
		...
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(mContext).setTitle(R.string.page_info)
				.setIcon(android.R.drawable.ic_dialog_info).setView(pageInfoView)
		...
		mSSLCertificateOnErrorDialog = createSslCertificateDialog(cert, error)
				.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						WebViewClassic.fromWebView(view).getWebViewClient().onReceivedSslError(view, handler, error);
					}
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						showPageInfo(mController.getTabControl().getTabFromView(view), true, error.getUrl());
					}
				}).setOnCancelListener(new DialogInterface.OnCancelListener() {
					public void onCancel(DialogInterface dialog) {
						mSSLCertificateOnErrorDialog = null;
	 ...
	 * null, text is added to state that the certificae is valid and the icon is
	 * set accordingly. If error is non-null, it must relate to the supplied
	...
	private AlertDialog.Builder createSslCertificateDialog(SslCertificate certificate, SslError error) {
		...
		return new AlertDialog.Builder(mContext).setTitle(com.android.internal.R.string.ssl_certificate).setIcon(iconId)
