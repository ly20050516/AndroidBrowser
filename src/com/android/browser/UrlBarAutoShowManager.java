...
/**
 * Helper class to manage when to show the URL bar based off of touch input, and
 * when to begin the hide timer.
 */
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsTracking && !mHasTriggered) {
				...
				if (ady > mSlop) {
					...
					if (dy > mSlop && angle > V_TRIGGER_ANGLE && !mUi.isTitleBarShowing()
							&& (web.getVisibleTitleHeight() == 0 || (!mIsScrolling && web.getScrollY() > 0))) {
