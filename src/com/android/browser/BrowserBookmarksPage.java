 ...
 */
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener, LoaderManager.LoaderCallbacks<Cursor>,
		BreadCrumbView.Controller, OnChildClickListener {
	...
	public boolean handleContextItem(int itemId, int groupPosition, int childPosition) {
		...
		case R.id.share_link_context_menu_id: {
			...
			Controller.sharePage(activity, cursor.getString(BookmarksLoader.COLUMN_INDEX_TITLE),
					cursor.getString(BookmarksLoader.COLUMN_INDEX_URL), getBitmap(cursor, BookmarksLoader.COLUMN_INDEX_FAVICON),
	...
	boolean canEdit(Cursor c) {
		...
		return type == BrowserContract.Bookmarks.BOOKMARK_TYPE_BOOKMARK || type == BrowserContract.Bookmarks.BOOKMARK_TYPE_FOLDER;
	}
	...
	public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
		...
		Cursor cursor = adapter.getItem(childPosition);
		boolean isFolder = cursor.getInt(BookmarksLoader.COLUMN_INDEX_IS_FOLDER) != 0;
		if (mCallbacks != null && mCallbacks.onBookmarkSelected(cursor, isFolder)) {
			return true;
		}
	...
	class OpenAllInTabsTask extends AsyncTask<Void, Void, Cursor> {
		...
		protected Cursor doInBackground(Void... params) {
			...
			if (c == null)
				return null;
			return c.getContentResolver().query(BookmarkUtils.getBookmarksUri(c), BookmarksLoader.PROJECTION,
					BrowserContract.Bookmarks.PARENT + "=?", new String[] { Long.toString(mFolderId) }, null);
		}
	...
	private static class LookupBookmarkCount extends AsyncTask<Long, Void, Integer> {
		...
		protected Integer doInBackground(Long... params) {
			...
			Cursor c = mContext.getContentResolver().query(uri, null, BrowserContract.Bookmarks.PARENT + "=?",
	...
	static class AccountsLoader extends CursorLoader {
		...
		static String[] ACCOUNTS_PROJECTION = new String[] { Accounts.ACCOUNT_NAME, Accounts.ACCOUNT_TYPE };

		public AccountsLoader(Context context) {
			super(context, Accounts.CONTENT_URI.buildUpon().appendQueryParameter(BrowserProvider2.PARAM_ALLOW_EMPTY_ACCOUNTS, "false")
					.build(), ACCOUNTS_PROJECTION, null, null, null);
		}
