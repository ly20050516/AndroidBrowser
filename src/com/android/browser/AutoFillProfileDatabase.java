...
public class AutoFillProfileDatabase {
	...
	private static class AutoFillProfileDatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + PROFILES_TABLE_NAME + " (" + Profiles._ID + " INTEGER PRIMARY KEY," + Profiles.FULL_NAME
					+ " TEXT," + Profiles.EMAIL_ADDRESS + " TEXT," + Profiles.COMPANY_NAME + " TEXT," + Profiles.ADDRESS_LINE_1 + " TEXT,"
					+ Profiles.ADDRESS_LINE_2 + " TEXT," + Profiles.CITY + " TEXT," + Profiles.STATE + " TEXT," + Profiles.ZIP_CODE
					+ " TEXT," + Profiles.COUNTRY + " TEXT," + Profiles.PHONE_NUMBER + " TEXT" + " );");
		}
	...
	public void addOrUpdateProfile(final int id, AutoFillProfile profile) {
		final String sql = "INSERT OR REPLACE INTO " + PROFILES_TABLE_NAME + " (" + Profiles._ID + "," + Profiles.FULL_NAME + ","
				+ Profiles.EMAIL_ADDRESS + "," + Profiles.COMPANY_NAME + "," + Profiles.ADDRESS_LINE_1 + "," + Profiles.ADDRESS_LINE_2
				+ "," + Profiles.CITY + "," + Profiles.STATE + "," + Profiles.ZIP_CODE + "," + Profiles.COUNTRY + ","
				+ Profiles.PHONE_NUMBER + ") VALUES (?,?,?,?,?,?,?,?,?,?,?);";
		final Object[] params = { id, profile.getFullName(), profile.getEmailAddress(), profile.getCompanyName(),
				profile.getAddressLine1(), profile.getAddressLine2(), profile.getCity(), profile.getState(), profile.getZipCode(),
				profile.getCountry(), profile.getPhoneNumber() };
		getDatabase(true).execSQL(sql, params);
	}

	public Cursor getProfile(int id) {
		final String[] cols = { Profiles.FULL_NAME, Profiles.EMAIL_ADDRESS, Profiles.COMPANY_NAME, Profiles.ADDRESS_LINE_1,
				Profiles.ADDRESS_LINE_2, Profiles.CITY, Profiles.STATE, Profiles.ZIP_CODE, Profiles.COUNTRY, Profiles.PHONE_NUMBER };
