...
public class SuggestionsAdapter extends BaseAdapter implements Filterable, OnClickListener {
	...
	private static final String[] COMBINED_PROJECTION = { OmniboxSuggestions._ID, OmniboxSuggestions.TITLE, OmniboxSuggestions.URL,
	...
	public SuggestionsAdapter(Context ctx, CompletionListener listener) {
		...
		mLinesLandscape = mContext.getResources().getInteger(R.integer.max_suggest_lines_landscape);
	...
	private void bindView(View view, SuggestItem item) {
		...
		ic2.setVisibility(((TYPE_SUGGEST == item.type) || (TYPE_SEARCH == item.type)) ? View.VISIBLE : View.GONE);
	...
	class CombinedCursor extends CursorSource {
		...
		public SuggestItem getItem() {
			if ((mCursor != null) && (!mCursor.isAfterLast())) {
				...
				return new SuggestItem(getTitle(title, url), getUrl(title, url), isBookmark ? TYPE_BOOKMARK : TYPE_HISTORY);
			}
			return null;
		}
		...
		public void runQuery(CharSequence constraint) {
			...
			ub.appendQueryParameter(BrowserContract.PARAM_LIMIT, Integer.toString(Math.max(mLinesLandscape, mLinesPortrait)));
			mCursor = mContext.getContentResolver().query(ub.build(), COMBINED_PROJECTION, selection, (constraint != null) ? args : null,
					null);
			if (mCursor != null) {
				mCursor.moveToFirst();
			}
	...
	class SuggestCursor extends CursorSource {
		...
		public SuggestItem getItem() {
			if (mCursor != null) {
				String title = mCursor.getString(mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_1));
				String text2 = mCursor.getString(mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2));
				String url = mCursor.getString(mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2_URL));
				String uri = mCursor.getString(mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_DATA));
				int type = (TextUtils.isEmpty(url)) ? TYPE_SUGGEST : TYPE_SUGGEST_URL;
				SuggestItem item = new SuggestItem(title, url, type);
				item.extra = mCursor.getString(mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA));
				return item;
			}
			return null;
		}
