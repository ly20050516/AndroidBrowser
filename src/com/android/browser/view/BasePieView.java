...
public abstract class BasePieView implements PieMenu.PieView {
	...
	/**
	 * this will be called before the first draw call needs to set top, left,
	 * width, height
	 */
	@Override
	...
	public boolean onTouchEvent(MotionEvent evt) {
		...
		if ((evtx < mLeft) || (evtx >= mLeft + mWidth) || (evty < mTop) || (evty >= mTop + mHeight)) {
			return false;
		}
		switch (action) {
		case MotionEvent.ACTION_MOVE:
			View v = mViews.get(mCurrent);
			setCurrent(Math.max(0, Math.min(mViews.size() - 1, findChildAt(evty))));
