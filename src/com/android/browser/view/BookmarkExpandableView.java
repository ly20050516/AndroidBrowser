...
public class BookmarkExpandableView extends ExpandableListView implements BreadCrumbView.Controller {
	...
	private OnClickListener mChildClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (mAdapter.getGroupCount() <= groupPosition || mAdapter.mChildren.get(groupPosition).getCount() <= childPosition) {
				return;
			}
			long id = mAdapter.mChildren.get(groupPosition).getItemId(childPosition);
			if (mOnChildClickListener != null) {
				mOnChildClickListener.onChildClick(BookmarkExpandableView.this, v, groupPosition, childPosition, id);
			}
		}
	};
	...
	class BookmarkAccountAdapter extends BaseExpandableListAdapter {
		...
		HashMap<Integer, BreadCrumbView> mBreadcrumbs = new HashMap<Integer, BreadCrumbView>();
		...
		public BreadCrumbView getBreadCrumbView(int groupPosition) {
			...
			if (crumbs == null) {
				crumbs = (BreadCrumbView) mInflater.inflate(R.layout.bookmarks_header, null);
				...
				crumbs.pushView(bookmarks, false, BrowserContract.Bookmarks.CONTENT_URI_DEFAULT_FOLDER);
