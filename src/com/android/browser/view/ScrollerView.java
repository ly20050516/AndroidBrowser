...
public class ScrollerView extends FrameLayout {
	...
	public int getMaxScrollAmount() {
		return (int) (MAX_SCROLL_FACTOR * (mHorizontal ? (mRight - mLeft) : (mBottom - mTop)));
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsOrthoDragged) {
				...
				if (mHorizontal) {
					if (overScrollBy(deltaY, 0, mScrollX, 0, range, 0, mOverscrollDistance, 0, true)) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mIsOrthoDragged) {
				...
				int initialVelocity = mHorizontal ? (int) velocityTracker.getXVelocity(mActivePointerId) : (int) velocityTracker
	...
	private int getScrollRange() {
		...
		if (getChildCount() > 0) {
			View child = getChildAt(0);
			if (mHorizontal) {
				scrollRange = Math.max(0, child.getWidth() - (getWidth() - mPaddingRight - mPaddingLeft));
			} else {
	 ...
	 * 
	 * @param topFocus
	 *            look for a candidate at the top of the bounds if topFocus is
	 *            true, or at the bottom of the bounds if topFocus is false
	 * @param top
	...
	private View findFocusableViewInMyBounds(final boolean topFocus, final int top, View preferredFocusable) {
		 ...
		 * by 2.
		 */
		final int fadingEdgeLength = (mHorizontal ? getHorizontalFadingEdgeLength() : getVerticalFadingEdgeLength()) / 2;
		final int topWithoutFadingEdge = top + fadingEdgeLength;
		final int bottomWithoutFadingEdge = top + (mHorizontal ? getWidth() : getHeight()) - fadingEdgeLength;

		if ((preferredFocusable != null)
				&& ((mHorizontal ? preferredFocusable.getLeft() : preferredFocusable.getTop()) < bottomWithoutFadingEdge)
	...
	protected void measureChild(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
		...
		if (mHorizontal) {
			childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec, mPaddingTop + mPaddingBottom, lp.height);
	...
	protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec,
		...
		int childHeightMeasureSpec;
		if (mHorizontal) {
			childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec, mPaddingTop + mPaddingBottom + lp.topMargin
			...
			childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec, mPaddingLeft + mPaddingRight + lp.leftMargin
	...
	public void fling(int velocityY) {
		if (getChildCount() > 0) {
			if (mHorizontal) {
				...
				mScroller.fling(mScrollX, mScrollY, velocityY, 0, 0, Math.max(0, right - width), 0, 0, width / 2, 0);
			} else {
