 ...
 * stored is the set of bookmarks. It's okay if I/O exceptions are thrown out of
 * the agent; the calling code handles it and the backup operation simply fails.
 * 
 * @hide
 */
public class BrowserBackupAgent extends BackupAgent {
	...
	/**
	 * this version num MUST be incremented if the flattened-file schema ever
	 * changes
	 */
	static final int BACKUP_AGENT_VERSION = 0;

	/**
	 * This simply preserves the existing state as we now prefer Chrome Sync to
	 * handle bookmark backup.
	 */
	@Override
	...
	/**
	 * Restore from backup -- reads in the flattened bookmark file as supplied
	 * from the backup service, parses that out, and rebuilds the bookmarks
	 * table in the browser database from it.
	 */
	@Override
			...
			while (data.readNextHeader()) {
				if (BOOKMARK_KEY.equals(data.getKey())) {
					...
					try {
						...
						// Read all the bookmarks, then process later -- if we
						// can't read
						// all the data successfully, we don't touch the
						// bookmarks table
						...
						// Okay, we have all the bookmarks -- now see if we need
						// to add
						// them to the browser's database
						int N = bookmarks.size();
						int nUnique = 0;
						if (DEBUG)
							Log.v(TAG, "Restoring " + N + " bookmarks");
						String[] urlCol = new String[] { Bookmarks.URL };
						for (int i = 0; i < N; i++) {
							Bookmark mark = bookmarks.get(i);

							// Does this URL exist in the bookmark table?
							Cursor cursor = getContentResolver().query(Bookmarks.CONTENT_URI, urlCol, Bookmarks.URL + " == ?",
