...
public class BrowserSettings implements OnSharedPreferenceChangeListener, PreferenceKeys {
	...
	private static final String DESKTOP_USERAGENT = "Mozilla/5.0 (X11; " + "Linux x86_64) AppleWebKit/534.24 (KHTML, like Gecko) "
	...
	private static final String IPAD_USERAGENT = "Mozilla/5.0 (iPad; U; " + "CPU OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 "
			+ "(KHTML, like Gecko) Version/4.0.4 Mobile/7B367 Safari/531.21.10";

	private static final String FROYO_USERAGENT = "Mozilla/5.0 (Linux; U; "
			+ "Android 2.2; en-us; Nexus One Build/FRF91) AppleWebKit/533.1 " + "(KHTML, like Gecko) Version/4.0 Mobile Safari/533.1";

	private static final String HONEYCOMB_USERAGENT = "Mozilla/5.0 (Linux; U; "
			+ "Android 3.1; en-us; Xoom Build/HMJ25) AppleWebKit/534.13 " + "(KHTML, like Gecko) Version/4.0 Safari/534.13";

	private static final String USER_AGENTS[] = { null, DESKTOP_USERAGENT, IPHONE_USERAGENT, IPAD_USERAGENT, FROYO_USERAGENT,
			HONEYCOMB_USERAGENT, };
	...
	private Runnable mSetup = new Runnable() {
		...
		public void run() {
			...
			// the cost of one cached page is ~3M (measured using nytimes.com).
			// For
			// low end devices, we only cache one page. For high end devices, we
			// try
			// to cache more pages, currently choose 5.
			if (ActivityManager.staticGetMemoryClass() > 16) {
				mPageCacheCapacity = 5;
			}
			mWebStorageSizeManager = new WebStorageSizeManager(mContext, new WebStorageSizeManager.StatFsDiskInfo(getAppCachePath()),
			...
			if (mPrefs.contains(PREF_TEXT_SIZE)) {
				/*
				 * Update from TextSize enum to zoom percent SMALLEST is 50%
				 * SMALLER is 75% NORMAL is 100% LARGER is 150% LARGEST is 200%
				 */
				switch (getTextSize()) {
				case SMALLEST:
					setTextZoom(50);
	...
	private void syncSetting(WebSettingsClassic settings) {
		...
		settings.setProperty(WebViewProperties.gfxInvertedScreen, useInverted ? "true" : "false");
		if (useInverted) {
			settings.setProperty(WebViewProperties.gfxInvertedScreenContrast, Float.toString(getInvertedContrast()));
		}

		if (isDebugEnabled()) {
			settings.setProperty(WebViewProperties.gfxEnableCpuUploadPath, enableCpuUploadPath() ? "true" : "false");
		}
	...
	public void setAutoFillProfile(AutoFillProfile profile, Message msg) {
		...
		// Auto-fill will reuse the same profile ID when making edits to the
		// profile,
	...
	public String getDefaultPreloadSetting() {
		String preload = Settings.Secure.getString(mContext.getContentResolver(), DEAULT_PRELOAD_SECURE_SETTING_KEY);
		if (preload == null) {
	 ...
	 * 
	 * @param time
	 *            The last time as System.currentTimeMillis that the browser was
	 *            started. This should be set to 0 if the last tab is closed.
	 */
	 ...
	 * 
	 * @param isPaused
	 *            Set to true When a pause is received or false after resuming.
	 */
